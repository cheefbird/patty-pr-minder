...
Deno.test("GitHub client core behavior", async (t) => {
  await t.step("fetchPR returns data and uses cache", async () => {
    ...
    await withStubbedFetch(
      ...
      async () => {
        ...
        assertEquals(callCount, 1, "cached response should avoid duplicate fetch");
      },
    );
  });

  await t.step("fetchPR returns null for missing PR and memoizes result", async () => {
    let callCount = 0;
    await withStubbedFetch(
      async () => {
        callCount += 1;
      ...
      async () => {
        ...
        const second = await client.fetchPR("cheefbird", "patty-pr-minder", 999);
  ...
  await t.step("fetchPR retries after rate limiting and respects reset header", async () => {
  ...
  await t.step("validateToken returns false when GitHub responds with 401", async () => {
    await withStubbedFetch(
      async (input) => {
  ...
  await t.step("getRateLimit returns latest headers after successful request", async () => {
    await withStubbedFetch(
      async () => {
        await Promise.resolve();
        return createJsonResponse(
